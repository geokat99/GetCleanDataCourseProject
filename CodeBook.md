...
we captured 3-axial linear Accelerometer and 3-axial angular velocity at a constant rate of 50Hz. 
...
The sensor Accelerometer signal, which has gravitational and body motion components, 
was separated using a Butterworth low-pass filter into body Accelerometer and gravity. 
...
From the initial data set were extracted only the measurements on the mean and standard deviation 
...
angle(Y,gravityMean)
angle(Z,gravityMean)
were not consider (by me) to fullfill the selection criteria. 
Actually that were not a measurement mean.
The rest of the variables with "mean()" or "std()" in their names were included in the new data file.
Activity and Subject variables were also included in the new data file.
Activity names were used instead of 1:6 values.
The data set was relabeled with descriptive variable names following the tidy data set rules, getting rid off 
from string-characters like "-()," and using full names instead of acronyms.
Finally a new data set with the average of each variable for each activity and subject was created.
